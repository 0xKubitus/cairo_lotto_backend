...
fn test_balance_checker() {
    ...
    // First, initialize the ERC20 with initial supply and to which account we use to mint the inital supply.
    ...
    set_contract_address(BALANCE_CHECKER_ADDR);
    assert(Draft::ensure_minimum_balance_of(EVE_ADDR) == true,
           'EVE balance not enough');
